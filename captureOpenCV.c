...
{
	//color : 1. 살색 2. 흰색 3. 빨간색 4. 노란색 5. 초록색 6.흰*노 mix defalut. 노란차선검출
	//color : 1. 살색 2. 흰색 3.검은색 4. 빨간색 5. 노란색 6. 초록색 7.흰*노 mix defalut. 노란차선검출
	NvMediaVideoSurfaceMap surfMap;
	unsigned int resWidth, resHeight;
	unsigned char y, u, v;
	int num;
...
int IsStop(IplImage* imgResult) {
	...
	printf("stop\n");
	//if (Stop_line() == 1){
	if (Stop_line() == 1 && stop_camera > 2) { ///stop_flag 의 계수 3 은 test따라 변경
		DesireSpeed_Write(0);
		stop_camera = 0;
		printf("stop!!!!!!\n");
		return 1;
	}
	for (y = 50; y < imgResult->height; y++) {
...
void Find_Center(IplImage* imgResult) {
	...
	if (line_founded == false)
		speed_1 = 130;
	else
		speed_1 = 50;
	DesireSpeed_Write(speed_1);
}
...
void After_Light(IplImage* imgResult, int direction) {
	int x, y;
	int white_line = 0;
	int flag = 0;
	CameraYServoControl_Write(1800);

	for (y = imgResult->height; y > 180; y--) {
		for (x = 0; x < imgResult->width; x++) {
			if (imgResult->imageData[y*(imgResult->widthStep) + x] == 255) {
				white_line++;
			}
...
/*
int emergencyStopRed(IplImage* imgResult) { // Find_Center보다 뒤에서 사용해야 급정거 효과 있음
	int x = 20, y = 30;
...
void *ControlThread(void *unused)
{
	...
	while (1)
	{
		...
		else if (count % 7 == 0) {//emergency stop
			color = 3;
			count = 1;
		}
		else {//노랑선
